...
function getNavItems(profile) {
  if (profile === PROFILES.ALUNO) return []
  if (profile === PROFILES.PRECEPTOR) return []
      ...
      <aside className="dashboard-sidebar">
        ...
        <div className="dashboard-user">
          <div className="dashboard-user-avatar">
            {user?.name?.split(' ').map((n) => n[0]).slice(0, 2).join('').toUpperCase() || 'CM'}
          </div>
          <div className="dashboard-user-info">
            <span className="dashboard-user-name">
              {user?.name || t('dashboard.userName')}
            </span>
          ...
          <div className="dashboard-header-actions">
            ...
            {isParams ? (
              ...
              <>
                ...
                <button
                  type="button"
                  className="dashboard-btn dashboard-btn--secondary dashboard-btn--header"
                  onClick={() => { logout(); navigate('/login'); }}
                  title="Sair"
                >
                  <span className="material-icons" style={{ marginRight: 4 }}>logout</span>
                  Sair
                </button>
              </>
            )}
